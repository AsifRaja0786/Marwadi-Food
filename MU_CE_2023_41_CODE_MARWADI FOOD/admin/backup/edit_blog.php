<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $val = ['', '', $date, '', '', '', '', '', '', '', '', '', '', '', $_POST['prev_userViewCount'], $_POST['prev_guestViewCount'], $_POST['prev_reportCount'], $_POST['prev_bookmarkCount'], $_POST['prev_upvoteCount'], $_POST['prev_downvoteCount']];
        $errorMsg = ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''];
        ...
        for ($i=1; $i<=$numOfTagAndImage; $i++) {
            ...
            if ($_FILES[$imgInputName]['error'] === 4) {
                ...
                if (!in_array($imgExtension, $validImgExtension)) {
                    ...
                    $newImgName = $_POST['blogId'] . "_" . $i . "." . $imgExtension;
                    $val[$i + 3] = $newImgName;
        ...
        if ($flag==true) {
            ...
            if (mysqli_query($con, $query)) {
                ...
                if (trim($row['prev_img1'])) {
                    unlink($folder.$row['prev_img1']);
                }
                if (trim($row['prev_img2'])) {
                    unlink($folder.$row['prev_img2']);
                }
                if (trim($row['prev_img3'])) {
                    unlink($folder.$row['prev_img3']);
                }
                if (trim($row['prev_img4'])) {
                    unlink($folder.$row['prev_img4']);
                }
                if (trim($row['prev_img5'])) {
                    unlink($folder.$row['prev_img5']);
                }

                // Success Message
                echo "<script>alert('Successfully Edited Blog');</script>";
            } else {
                // Error Message
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <form method="post" class="container form-horizontal" autocomplete="off" enctype="multipart/form-data" id="blogDetail" hidden=true>
                        ...
                        <!-- Hidden Fields -->
                        <input type="text" name="blogId" id="" value="<?= $selectedBlog ?>" hidden="true">
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <span class="form-control" id="content" style="height: auto;" contenteditable="true" required>
                                    <?php
                                        if ($rowCount) {
                                            echo trim($row['blogcon']);
                                        } 
                                    ?>
                                </span>
                                <!-- Store HTML content -->
                                <input type="text" hidden="true" name="blogContent" id="actualcontent">
                            </div>

                        </div>

                        <!-- Store previous photo name -->
                        <input type="text" hidden="true" name="prev_img1" value="<?= $row['img1'] ?>">
                        <input type="text" hidden="true" name="prev_img2" value="<?= $row['img2'] ?>">
                        <input type="text" hidden="true" name="prev_img3" value="<?= $row['img3'] ?>">
                        <input type="text" hidden="true" name="prev_img4" value="<?= $row['img4'] ?>">
                        <input type="text" hidden="true" name="prev_img5" value="<?= $row['img5'] ?>">

                        <!-- Store prvious counts -->
                        <input type="text" hidden="true" name="prev_userViewCount" value="<?= $row['userViewCount'] ?>">
                        <input type="text" hidden="true" name="prev_guestViewCount" value="<?= $row['guestViewCount'] ?>">
                        <input type="text" hidden="true" name="prev_reportCount" value="<?= $row['reportCount'] ?>">
                        <input type="text" hidden="true" name="prev_bookmarkCount" value="<?= $row['bookmarkCount'] ?>">
                        <input type="text" hidden="true" name="prev_upvoteCount" value="<?= $row['upvoteCount'] ?>">
                        <input type="text" hidden="true" name="prev_downvoteCount" value="<?= $row['downvoteCount'] ?>">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-2">
                                <input name="submit" value="Edit" type="submit" class="btn btn-primary form-control">
                            </div>
    ...
    <?php
        ...
        if (isset($_POST['search'])) {
            if ($rowCount) {
                ...
                echo "<script>document.getElementById('actualcontent').value = document.getElementById('content').innerHTML;</script>";
